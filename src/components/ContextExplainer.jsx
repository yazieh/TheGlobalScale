...
const EXPLAINERS = [
    ...
    },
    {
        ...
        icon: Globe,
        content: "Earning $10 in India buys more food than $10 in New York. We adjust all numbers for 'Purchasing Power Parity' (PPP) to compare actual standard of living, not just currency.",
        link: "https://data.worldbank.org/indicator/PA.NUS.PRVT.PP",
        linkText: "World Bank Data"
    },
    {
        ...
        linkText: "Visit WID.world"
    }
];

export default function ContextExplainer() {
    const [isLotteryExpanded, setIsLotteryExpanded] = useState(false);
            ...
            <div className="max-w-4xl mx-auto space-y-24">
                ...
                <div className="space-y-12">
                    ...
                    >
                        ...
                        <button
                            onClick={() => setIsLotteryExpanded(!isLotteryExpanded)}
                            className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-zinc-400 hover:text-[var(--color-accent)] transition-colors mt-4"
                        >
                            {isLotteryExpanded ? "Show Less" : "Visualize the Odds"}
                            <motion.div animate={{ rotate: isLotteryExpanded ? 180 : 0 }}>↓</motion.div>
                        </button>
                    </motion.div>

                    <motion.div
                        initial={false}
                        animate={{ height: isLotteryExpanded ? 'auto' : 0, opacity: isLotteryExpanded ? 1 : 0 }}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {EXPLAINERS.map((item, i) => (
                            <motion.div
                                ...
                                className="bg-white p-6 rounded-2xl border border-zinc-100 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
                            >
                                <div>
                                ...
                                <a
                                    ...
                                    className="text-xs font-bold text-[var(--color-accent)] hover:underline uppercase tracking-wide flex items-center gap-1"
                                >
                                    {item.linkText}
                                </a>
                            </motion.div>
